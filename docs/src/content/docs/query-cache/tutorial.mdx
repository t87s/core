...
Let's build a cache for a small app with users, posts, and comments. By the end of this page, you'll understand how QueryCache works, why the pieces fit together the way they do, and how to apply it to your own app. We'll go slow enough that it actually makes sense.
...
That's genuinely it. Query, cache, invalidate, repeat. The cache handles all the boring parts—storing values, checking freshness, managing TTLs—so you can focus on the parts of your app that aren't boring.

## Bonus: ad-hoc caching with primitives

Sometimes you want to cache something that doesn't fit neatly into a query definition. Maybe it's a computed aggregate, or a value from an external API, or something you just want to stash temporarily.

QueryCache exposes its underlying primitives for exactly this:

<Tabs>
  <TabItem label="TypeScript">
    ```typescript
    // Cache an arbitrary value with its own tags and TTL
    await cache.primitives.set('stats:users', userStats, {
      tags: [['users']],
      ttl: '5m',
    });
    ```
...
When you invalidate `users`, this cached stat goes stale too—even though it wasn't defined as a query. That's the nice thing about tags: everything that shares a tag shares a fate.
