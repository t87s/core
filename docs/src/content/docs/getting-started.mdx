...
t87s has two APIs: QueryCache and primitives. This quickstart uses the [QueryCache](/query-cache/schema/) API because it's pretty sweet, but in another world we could have started with [primitives](/primitives/) and you would have been an equally-happy reader.
...
If you want the full tour, start with the [QueryCache tutorial](/query-cache/tutorial/), then circle back to [Primitives](/primitives/) when you want sharper tools.
